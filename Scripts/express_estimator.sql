...
CREATE TABLE dm2014.uniscanex AS
     (SELECT ST_Difference(dm2014.merge_scanex.geom, dm2014.dg_re_airbus.geom) AS geom
        FROM dm2014.merge_scanex, dm2014.dg_re_airbus);
CREATE TABLE dm2014.unidg AS
     (SELECT ST_Difference(dm2014.merge_dg.geom, dm2014.scanex_re_airbus.geom) AS geom
        FROM dm2014.merge_dg, dm2014.scanex_re_airbus);
CREATE TABLE dm2014.unire AS
     (SELECT ST_Difference(dm2014.merge_re.geom, dm2014.dg_scanex_airbus.geom) AS geom
        FROM dm2014.merge_re, dm2014.dg_scanex_airbus);
CREATE TABLE dm2014.uniairbus AS
     (SELECT ST_Difference(dm2014.merge_airbus.geom, dm2014.dg_scanex_re.geom) AS geom
        FROM dm2014.merge_airbus, dm2014.dg_scanex_re);

/* Именование комбинаций */
ALTER TABLE merge_dg
 ADD COLUMN name varchar;
UPDATE merge_dg
   SET name = 'DigitalGlobe';

ALTER TABLE merge_scanex
 ADD COLUMN name varchar;
UPDATE merge_scanex
   SET name = 'SPOT от Сканэкса';

ALTER TABLE merge_re
 ADD COLUMN name varchar;
UPDATE merge_re
   SET name = 'RapidEye';

ALTER TABLE merge_airbus
 ADD COLUMN name varchar;
UPDATE merge_airbus
   SET name = 'Airbus (SPOT и Peiades)';

ALTER TABLE unidg
 ADD COLUMN name varchar;
UPDATE unidg
   SET name = 'уникальный DigitalGlobe';

ALTER TABLE uniscanex
 ADD COLUMN name varchar;
UPDATE uniscanex
   SET name = 'уникальный SPOT от Сканэкса';

ALTER TABLE unire
 ADD COLUMN name varchar;
UPDATE unire
   SET name = 'уникальный RapidEye';

ALTER TABLE uniairbus
 ADD COLUMN name varchar;
UPDATE uniairbus
   SET name = 'уникальный Airbus (SPOT и Peiades)';

ALTER TABLE dg_scanex
 ADD COLUMN name varchar;
UPDATE dg_scanex
   SET name = 'DigitalGlobe + SPOT от Сканэкса';

ALTER TABLE dg_re
 ADD COLUMN name varchar;
UPDATE dg_re
   SET name = 'DigitalGlobe + RapidEye';

ALTER TABLE dg_airbus
 ADD COLUMN name varchar;
UPDATE dg_airbus
   SET name = 'DigitalGlobe + Airbus (SPOT и Peiades)';

ALTER TABLE scanex_re
 ADD COLUMN name varchar;
UPDATE scanex_re
   SET name = 'SPOT от Сканэкса + RapidEye';

ALTER TABLE scanex_airbus
 ADD COLUMN name varchar;
UPDATE scanex_airbus
   SET name = 'SPOT от Сканэкса + Airbus (SPOT и Peiades)';

ALTER TABLE re_airbus
 ADD COLUMN name varchar;
UPDATE re_airbus
   SET name = 'RapidEye + Airbus (SPOT и Peiades)';

ALTER TABLE dg_scanex_re
 ADD COLUMN name varchar;
UPDATE dg_scanex_re
   SET name = 'DigitalGlobe + SPOT от Сканэкса + RapidEye';

ALTER TABLE dg_scanex_airbus
 ADD COLUMN name varchar;
UPDATE dg_scanex_airbus
   SET name = 'DigitalGlobe + SPOT от Сканэкса + Airbus (SPOT и Peiades)';

ALTER TABLE dg_re_airbus
 ADD COLUMN name varchar;
UPDATE dg_re_airbus
   SET name = 'DigitalGlobe + RapidEye + Airbus (SPOT и Peiades)';

ALTER TABLE scanex_re_airbus
 ADD COLUMN name varchar;
UPDATE scanex_re_airbus
   SET name = 'SPOT от Сканэкса + RapidEye + Airbus (SPOT и Peiades)';

ALTER TABLE dg_scanex_re_airbus
 ADD COLUMN name varchar;
UPDATE dg_scanex_re_airbus
   SET name = 'DigitalGlobe + SPOT от Сканэкса + RapidEye + Airbus (SPOT и Peiades)';